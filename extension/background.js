...
var stRevokeObjectURL;
if (URL && URL.createObjectURL && typeof URL.createObjectURL === 'function' && Blob) {
	stCreateObjectURL= function(text){return  URL.createObjectURL(new Blob([text], {type: 'text/html'}));}
	stRevokeObjectURL= function (text){return URL.revokeObjectURL(text);console.log("with URL");}
	
} else {
	stCreateObjectURL= function(text){return "data:text/html," + encodeURIComponent(text);}
	stRevokeObjectURL= function (text){console.log("without URL");}
}
var probBlobUrl = stCreateObjectURL(testfilecontent);
...
function dodownload (msg,tiddlywikilocations){	
	var objUrl = stCreateObjectURL(msg.txt);			
	...
	function(id) {chrome.downloads.onChanged.addListener(function hearchange(deltas){
		...
		chrome.storage.local.get({backuptw5:true,backuptwc:false,periodchoice:"day",period:[],backupdir:"backupdir",backedup:{}}, function(items) {
			...
			objUrlBkup = stCreateObjectURL(msg.txt);
			chrome.downloads.download({
					url: objUrlBkup,
    ...
    ///////////////////// disable discard //////////////////////////
    if (msg.type === "loaded") {
		console.log("savetiddlersbg: loaded");
